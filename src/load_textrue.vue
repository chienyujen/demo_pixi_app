...
async function newTexture(){
  ...
  await Assets.addBundle('scene1', assets1);
  const scene1 = await Assets.loadBundle('scene1', (progress)=>{
    console.log("加載進度", progress/1*100, "%");
  });
  
  // 建立 層級
  const container = new Container();
  // 建立一個 sprite
  const bon1 = Sprite.from(scene1.bon1);
  ...
  const bon2 = Sprite.from(scene1.bon2);
  bon2.scale.set(0.3);
